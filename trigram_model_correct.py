...
if __name__ == '__main__':
    ...
    for trigram in model.trigramcounts.keys():
        sum_probs[tuple(trigram[:2])] += model.smoothed_trigram_probability(trigram)
    count = 0
    for bigram in sum_probs:
        if abs(sum_probs[bigram] - 1) > 0.01:
            print(bigram)
            print(sum_probs[bigram])
            count += 1
        # assert abs(sum_probs[bigram] - 1) < 0.01
    print()
    print(count / len(sum_probs.keys()))
